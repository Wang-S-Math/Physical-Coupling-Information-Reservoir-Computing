...
def generate_coupled_lorenz_data(
    ...
    print(f"开始生成耦合洛伦兹数据 (XY乘积耦合, 使用阈值矩阵): {num_oscillators} 个振子, {steps} 个时间步...")
    ...
    for t in range(steps):
        def dynamics(state):
            ...
            # --- [核心耦合逻辑] ---
            # 耦合项 = sum(A_ij * y_j * x_i)

            # 1. 计算邻居 y 的加权和: sum_j (A_ij * y_j)
            # === 修改点：使用 adjacency_matrix_thresholded ===
            weighted_sum_y = adjacency_matrix_thresholded @ y

            # 2. 乘以自身的 x_i (逐元素相乘)
            coupling_term = weighted_sum_y * x
            # --------------------
...
if __name__ == "__main__":
    ...
    # 保存数据
    os.makedirs("../generate.data/llz.xy", exist_ok=True)  # 确保目录存在

    # 定义文件名
    file_ts = "../generate.data/llz.xy/700coupled_lorenz_data.xy.npy"
    file_adj = "../generate.data/llz.xy/700coupled_lorenz_adj.xy.npy"
    file_adj_thresholded = ("../generate.data/llz.xy/700coupled_lorenz_adj_thresholded.xy.npy")

    np.save(file_ts, ts_data)
    np.save(file_adj, true_weights)
    np.save(file_adj_thresholded, true_weights_thresholded)  # 保存阈值处理后的矩阵

    print(f"数据已保存至:")
    print(f"1. {file_ts}")
    print(f"2. {file_adj}")
    print(f"3. {file_adj_thresholded}")
